...
Physics::Physics(const ugdk::Vector2D& bounds) : bounds_(bounds) {}

Physics::~Physics() {
}

void Physics::Update(double dt, GameObject* owner) {
    ugdk::Vector2D map_size = bounds_;
    ugdk::Vector2D next_position = owner->world_position() + owner->velocity() * dt;
